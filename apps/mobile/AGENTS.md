...
- Avoid a global `src/types/` bucket for domain types. Export types from the owning module (lib, service, or feature folder) and prefer colocation over remote re-exports.
- Reserve `src/types/` for ambient declarations only (e.g. `assets.d.ts`), not shared domain models.
- Accepted exceptions: types that are standalone (no imports from other app modules), or types that stitch together a shared interface from multiple modules.
- Avoid clutter: keep components under 250 lines; keep at most 10 non-generated files per directory. Nest into appropriately named subfolders when a directory grows—unless a bundler or platform split (e.g. `*.web.tsx`) requires files to stay together or the files were generated by a tool that expects them like that
